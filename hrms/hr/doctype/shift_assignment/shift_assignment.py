	...
	return frappe._dict(
		{
			...
			"shift_attendance_date" : for_timestamp,
...
from erpnext.setup.doctype.employee.employee import get_shift_based_on_shift_cycle

# check whether any approved shift requests are there in Shift Assignment table
def glovis_get_shifts_for_date(employee: str, for_timestamp: datetime) -> List[Dict[str, str]]:
	"""Returns list of shifts with details for given date"""
	assignment = frappe.qb.DocType("Shift Assignment")
...
# 
def glovis_get_shift_for_timestamp(employee: str, for_timestamp: datetime) -> Dict:
	# get any requested shifts
	shifts = glovis_get_shifts_for_date(employee, for_timestamp)
	# print("getting assigned Shifts",shifts)
	if shifts:
		for entry in shifts:
			shift_details = glovis_get_shift_details(entry.shift_type, for_timestamp=for_timestamp)
			return shift_details
	# if no request approvals found return {}
	return {}

def glovis_get_employee_shift(
	employee: str,
	for_timestamp: datetime = None
) -> Dict:
	...
	shift_details = glovis_get_shift_for_timestamp(employee, for_timestamp)

	if len(shift_details)>0:
		# print("\n\n\n\n Case 1 Assigned Shift Details",shift_details)
		# it has shift requests on that date,but check whether it's valid shift or not
		return shift_details
	
	else:
		# if shift assignment is not found, consider shift rotation
		# print("\n-----# if shift assignment is not found, consider shift rotation------\n")
		shift_cycle_ = frappe.db.get_value("Employee", employee, "shift_cycle", cache=True)
		shift_ = get_shift_based_on_shift_cycle(shift_cycle_,for_timestamp)
		# print("\n\n\n\nCase 2 shift rotation ----",shift_)
		shift_details = glovis_get_shift_details(shift_, for_timestamp)
		# print("\nCase 2 Rotation Shift Details ",shift_details)
		
		# print("\nshift_details.actual_start---",shift_details.actual_start)
		# print("\nshift_details.actual_end---",shift_details.actual_end)

		return shift_details


def glovis_get_employee_shift_timings(
	employee: str, for_timestamp: datetime = None
) -> List[Dict]:
	"""Returns previous shift, current/upcoming shift, next_shift for the given timestamp and employee"""
	if for_timestamp is None:
		for_timestamp = now_datetime()
	...
	# write and verify a test case for midnight shift.
	prev_shift = curr_shift = None

	# ******************** current shift details ******************* #

	# print("curr for_timestamp-----",for_timestamp)
	curr_shift_details = glovis_get_employee_shift(employee, for_timestamp)

	if (
			get_datetime(curr_shift_details.actual_start)
			<= get_datetime(for_timestamp)
			<= get_datetime(curr_shift_details.actual_end)
	):
			# print("\nhe came on assigned Current shift")
			pass
	else:
			# print("\nhe doesn't come on assigned  Current shift")
			curr_shift_details = {}

	# print("\nfor_timestamp----",for_timestamp)
	# print("\n\ncurr_shift----",curr_shift_details)

	# ******************** previous shift details ******************* #

	# print("prev for_timestamp-----",for_timestamp + timedelta(days=-1))
	prev_shift_details = glovis_get_employee_shift(employee, for_timestamp + timedelta(days=-1))

	if (
			get_datetime(prev_shift_details.actual_start)
			<= get_datetime(for_timestamp)
			<= get_datetime(prev_shift_details.actual_end)
	):
			# print("\nhe came on assigned Previous shift")
			pass
	else:
			# print("\nhe doesn't come on assigned Previous shift")
			prev_shift_details = {}

	# print("\nfor_timestamp----",for_timestamp)
	# print("\n\nprev_shift----",prev_shift_details)

	return prev_shift_details, curr_shift_details

def glovis_get_actual_start_end_datetime_of_shift(
	employee: str, for_timestamp: datetime
) -> Dict:
	"""Returns a Dict containing shift details with actual_start and actual_end datetime values
	Here 'actual' means taking into account the "begin_check_in_before_shift_start_time" and "allow_check_out_after_shift_end_time".
	Empty Dict is returned if the timestamp is outside any actual shift timings.
	...
	default shift in employee master if no shift assignment is found
	"""
	shift_timings_as_per_timestamp = glovis_get_employee_shift_timings(
		employee, for_timestamp
	)
	return get_exact_shift(shift_timings_as_per_timestamp, for_timestamp)


def glovis_get_shift_details(shift_type_name: str, for_timestamp: datetime = None) -> Dict:
	"""Returns a Dict containing shift details with the following data:
	'shift_type' - Object of DocType Shift Type,
	'start_datetime' - datetime of shift start on given timestamp,
	'end_datetime' - datetime of shift end on given timestamp,
	...
	if shift_type.is_start_time_today and not shift_type.is_end_time_today:
		# start time is today, end time is on next day
		# print("# start time is today, end time is on next day")
		start_datetime = datetime.combine(for_timestamp, datetime.min.time()) + shift_type.start_time
		for_timestamp = for_timestamp + timedelta(days=1)
		end_datetime = datetime.combine(for_timestamp, datetime.min.time()) + shift_type.end_time
		
	elif not shift_type.is_start_time_today and not shift_type.is_end_time_today:
		# start time is next day, end time is on next day
		# print("\n # start time is next day, end time is on next day")
		# print("\nshift_type.start_time -----",shift_type.start_time)
		# print("\nfor_timestamp----",for_timestamp)
		# print("\nfor_timestamp + timedelta(days=1)----",for_timestamp + timedelta(days=1))
		...
		# start and end timings fall on the same day
		# print("# start and end timings fall on the same day")
		start_datetime = datetime.combine(for_timestamp, datetime.min.time()) + shift_type.start_time
		end_datetime = datetime.combine(for_timestamp, datetime.min.time()) + shift_type.end_time
	...
	return frappe._dict(
		{
			"shift_type": shift_type,
			"shift_attendance_date" : shift_attendance_date,
			"start_datetime": start_datetime,
			"end_datetime": end_datetime,
			"actual_start": actual_start,
			"actual_end": actual_end,
