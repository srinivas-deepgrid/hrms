...
frappe.ui.form.on('Compensatory Leave Request', {
	...
	date: function(frm){
		...
		if(frm.doc.date){
			frm.set_value("work_from_date",frm.doc.date);
			frm.set_value("work_end_date",frm.doc.date);
			refresh_field("work_from_date");
			refresh_field("work_end_date");
			// console.log(frm.doc.work_from_date);
			// console.log(frm.doc.work_end_date);
			frm.trigger("days")
			const expiryDate = frappe.datetime.add_days(frm.doc.date, 121);
		...
		if(frm.doc.date && frm.doc.employee && frm.doc.leave_type){
			
			frappe.call({
				method: "hrms.hr.doctype.compensatory_leave_request.compensatory_leave_request.get_compensatory_days",
				args: {
