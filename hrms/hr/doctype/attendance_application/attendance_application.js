frappe.ui.form.on('Attendance Application', {
	setup: function(frm) {
	...
	date:function(frm){
		if(frm.doc.employee && frm.doc.date){
			frappe.call({
				...
				callback: function(r) {
					console.log("Attendance status: " + r.message);

				  if(r.message === "Missed Punch" || r.message === "Permission"){
						frm.set_value("previous_attendance_status",r.message);
						refresh_field("previous_attendance_status");
					}else if (r.message){
						frm.set_value("previous_attendance_status","");
						refresh_field("previous_attendance_status");
						frappe.throw(__("Attendance Record Status Found is - "+ `<b>${r.message}</b>` +" on the Selected Date! Attendance Status Should be " + `<b>Permission/Missed Punch</b>`));
					}else{
						frm.set_value("previous_attendance_status","");
						refresh_field("previous_attendance_status");
			  ...
			  frappe.call({
				method: "hrms.hr.doctype.attendance_application.attendance_application.get_attendance_in_time_out_time_shift",
				args: {
					employee: frm.doc.employee,
					attendance_date: frm.doc.date
				},
				callback: function(r) {
				  ...
				  if(r.message){
					...
					frm.set_value("previous_in_time",r.message[0]);
					frm.set_value("out_time",r.message[1]);
					frm.set_value("previous_out_time",r.message[1]);
					frm.set_value("shift",r.message[2]);
					refresh_field("in_time");
					refresh_field("previous_in_time");
					refresh_field("out_time");
					refresh_field("previous_out_time");
					refresh_field("shift")
					}else{
					frm.set_value("in_time","");
					frm.set_value("previous_in_time","");
					frm.set_value("out_time","");
					frm.set_value("previous_out_time","");
					frm.set_value("shift","");
					refresh_field("in_time");
					refresh_field("previous_in_time");
					refresh_field("out_time");
