...
class LeaveApplication(Document):
	...
	def create_or_update_attendance(self, attendance_name, date):
		...
		if attendance_name:
			...
			# print('doc.count---',doc.name,doc.count)
			new_count = new_count + doc.count
			if doc.status != status:
				doc.db_set({"status": status, "leave_type": self.leave_type, "leave_application": self.name,"count":new_count})
			else:
				doc.db_set({"status": status, "leave_type_2": self.leave_type, "leave_application_2": self.name,"count":new_count})
		else:
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			attendance = frappe.db.sql(
				...
				and (attendance_date between %s and %s) and docstatus < 2 and status in ('On Leave')""",
				(self.employee, self.from_date, self.to_date),
				as_dict=1,
			)

			if attendance:
				# print('attendance---cancel---',attendance)
				for attendance_record in attendance:
					attendance_name = attendance_record.name
					# print('attendance_name---cancel---',attendance_name)
					frappe.db.set_value("Attendance", {"name": attendance_name}, {
						"docstatus":2,
						"count" : 0
						}
					)
	...
	def validate_block_days(self):
		...
		if block_dates and (self.status == 'Approved by Reporting Incharge' or self.status == "Approved by HR" or self.status == "Approved by HOD" or self.status == "Approved by CHO"):
			frappe.throw(_("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError)
	...
	def validate_balance_leaves(self):
		if self.from_date and self.to_date:
			...
			if not is_lwp(self.leave_type):
				...
				if (self.status not in status_list) and (
					leave_balance_for_consumption < self.total_leave_days or not leave_balance_for_consumption
				):
					self.show_insufficient_balance_message(leave_balance_for_consumption)
	...
	def validate_leave_overlap(self):
		...
		# edited and status in ('Open', 'Approved')
		for d in frappe.db.sql(
			"""
			select
				name, leave_type, posting_date, from_date, to_date, total_leave_days, half_day_date
			from `tabLeave Application`
			where employee = %(employee)s and docstatus < 2 and status in ('Approved by Reporting Incharge', 'Approved by HOD', 'Approved by CHO', 'Approved by HR', 'Awaiting Reporting Incharge Approval', 'Awaiting HOD Approval', 'Awaiting CHO Approval', 'Draft')
			...
			if (
				...
				# print('total_leaves_on_half_day---',total_leaves_on_half_day)
		...
		attendance = frappe.db.sql(
			"""select name from `tabAttendance` where employee = %s and (attendance_date between %s and %s)
					and status in ('Half Day','Absent','On Leave') and docstatus = 1""",
			(self.employee, self.from_date, self.to_date),
		)

		if attendance:
			# print('validate_leave_overlap-attendance---',attendance)
			for attendance_record in attendance:
				attendance_name = attendance_record[0]
				doc = frappe.get_doc("Attendance",attendance_name)
				# print('doc---atttendance---',doc.count,doc.name,doc.attendance_date,doc.status,'\n\n')
				if (cint(self.half_day) == 1):
					application_leave_count = 0.5
				else:
					application_leave_count = 1			
				
				total_count = doc.count + application_leave_count
				if total_count > 1:
					if doc.count == 0.5:
						frappe.throw(_("Please Select Half Day!,Your Attendance Status is {0} on {1}").format(frappe.bold(doc.status),frappe.bold(doc.attendance_date)))
					elif doc.count == 1:
						frappe.throw(_("You have already taken leave on {0}").format(frappe.bold(doc.attendance_date)))
	...
	def get_total_leaves_on_half_day(self):
		...
		leave_count_on_half_day_date = frappe.db.sql(
			...
			and docstatus < 2
			and status in ('Approved by Reporting Incharge','Approved by HOD','Approved by CHO','Approved by HR','Awaiting Reporting Incharge Approval','Awaiting HOD Approval','Awaiting CHO Approval','Draft')
			and half_day = 1
		...
		# print('leave_count_on_half_day_date----',leave_count_on_half_day_date)
	...
	def validate_attendance(self):
		attendance = frappe.db.sql(
			"""select name from `tabAttendance` where employee = %s and (attendance_date between %s and %s)
					and status in ('Present' ,'Missed Punch' ,'Permission', 'Work From Home') and docstatus = 1""",
			(self.employee, self.from_date, self.to_date),
		)
		...
		if attendance:
			for attendance_record in attendance:
				attendance_name = attendance_record[0]
				doc = frappe.get_doc("Attendance",attendance_name)
				if ((doc.status == 'Present') or (doc.status == 'Work From Home')):
					frappe.throw(
						_("Attendance for employee {0} is already marked for this day").format(self.employee),
						AttendanceAlreadyMarkedError,
					)
				elif ((doc.status == 'Missed Punch') or (doc.status == 'Permission')):
					frappe.throw(
						_("Attendance for employee {0} is {1} , Please Go to Attendance Application to Regularize").format(self.employee,doc.status),
						AttendanceAlreadyMarkedError,
					)
	...
	def create_leave_ledger_entry(self, submit=True):
		...
		if (self.status not in status_list) and submit:
			return
...
def get_leaves_pending_approval_for_period(
	...
	leaves = frappe.get_all(
		...
		filters={"employee": employee, "leave_type": leave_type, "status": "Draft"},
		or_filters={
...
def get_approved_leaves_for_period(employee, leave_type, from_date, to_date):
	...
	query = (
		...
		.where(
			...
			& (LeaveApplication.status.isin(status_list))
			& (
...
def check_schedular():
	...
	print('\n\n\n\n\n\n\n')
	frappe.msgprint("Schecdular is working!")
