...
from hrms.hr.doctype.shift_assignment.shift_assignment import (
	...
	glovis_get_actual_start_end_datetime_of_shift,
)
...
class EmployeeCheckin(Document):
	...
	def fetch_shift(self):
		...
		# print("STARTING--------get_datetime(self.time)---\n",get_datetime(self.time))
		
		shift_actual_timings = glovis_get_actual_start_end_datetime_of_shift(
			self.employee, get_datetime(self.time)
		)
		# print("shift_actual_timings----",shift_actual_timings)
		if shift_actual_timings:
			...
			if not self.attendance:
				...
				self.shift_attendance_date = shift_actual_timings.shift_attendance_date
			...
			self.shift = None
			self.shift_attendance_date = None
...
def add_log_based_on_employee_field(
	...
	doc.employee = employee_field_value
	doc.employee_name = employee_field_value
	doc.time = timestamp
	...
	shift_actual_timings = glovis_get_actual_start_end_datetime_of_shift(
		employee_field_value, get_datetime(timestamp)
	)

	# print("shift_actual_timings---",shift_actual_timings)
	# print("STARTING--------get_datetime(timestamp)---\n",get_datetime(timestamp))

	if shift_actual_timings:
		doc.shift = shift_actual_timings.shift_type.name
		doc.shift_attendance_date = shift_actual_timings.shift_attendance_date
		...
		doc.shift = None
		doc.shift_attendance_date = None
...
def mark_attendance_and_link_log(
	...
	"""
	# print("mark_attendance_and_link_log--",logs)
	# print("\n\n\n")
	log_names = [x.name for x in logs]
	employee = logs[0].employee

	# print("log_names---",log_names)
	...
	if attendance_status == "Skip":
	...
	elif attendance_status in ("Present", "Absent", "Half Day","Missed Punch","Permission"):
		...
		count = 0
		if ((attendance_status == "Present") or (attendance_status == "Work From Home")):
			count = 1
		...
		if not duplicate and not overlapping:
			...
			if attendance_status == "Missed Punch":
				attendance.add_comment(
					text=_("Employee was marked Missed Punch for having only ONE Biometric Record")
				)
			
			if attendance_status == "Permission":
				attendance.add_comment(
					text=_("Employee was marked Absent for not meeting the working hours threshold.")
				)
...
def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type):
	...
	if check_in_out_type == "Alternating entries as IN and OUT during the same shift":
		in_time = logs[0].time
		if len(logs) >= 2:
			if time_diff_in_hours(logs[0].time ,logs[-1].time)>=1:
