...
def get_approvers(doctype, txt, searchfield, start, page_len, filters):
	print("---------get_approvers got called--------")
	...
	# shift_request_approver is related to Department doc
	...
	print("Printing departmemt_approvers__________",approvers)
	...
	field_name = "Reporting Incharge"
	if len(approvers) == 0:
		error_msg = _("Please set Department for the Employee: {0}").format(
		 frappe.bold(employee.department)
		)
		
		if department_list:
			error_msg = " " + _("NO Reporting Incharge for Department: {0}").format(frappe.bold(employee_department))
		frappe.throw(error_msg, title=_("{0} Missing").format(_(field_name)))
