...
def get_message() -> str:
	message = ""
	colors = ["green", "red", "orange", "green", "#318AD8", "", "","red","purple"]
...
def get_chart_data(attendance_map: Dict, filters: Filters) -> Dict:
	...
	for day in days:
		...
		total_absent_on_day = total_leaves_on_day = total_present_on_day = total_missedPunches_on_day = total_Permissions_on_day = total_on_day =0
		...
		for employee, attendance_dict in attendance_map.items():
			for shift, attendance in attendance_dict.items():
				...
				elif attendance_on_day == "Missed Punch":
					total_missedPunches_on_day += 1
				elif attendance_on_day == "Permission":
					total_Permissions_on_day += 1
				

		absent.append(total_absent_on_day)
		present.append(total_present_on_day)
		leave.append(total_leaves_on_day)
		missedPunch.append(total_missedPunches_on_day)
		permission.append(total_Permissions_on_day)

		labels.append("Total")
		total_on_day = total_absent_on_day + total_present_on_day + total_leaves_on_day + total_missedPunches_on_day + total_Permissions_on_day
		total.append(total_on_day)
	...
	return {
		"data": {
			...
			"datasets": [
				...
				{"name": "Missed Punch", "values": missedPunch},
				{"name": "Permission", "values": permission},
				{"name" : "Total","values" : total}
			],
		},
		"type": "line",
		"colors": ["red", "green", "blue","red","purple","brown"],
	}
