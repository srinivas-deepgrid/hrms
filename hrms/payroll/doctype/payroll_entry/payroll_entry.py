...
class PayrollEntry(Document):
	...
	def make_filters(self):
		...
		filters["parent_department"] = self.parent_department
	...
	def fill_employee_details(self):
		...
		if not employees:
			...
			if self.parent_department:
				error_msg += "<br>" + _("Parent Department: {0}").format(frappe.bold(self.parent_department))
			if self.department:
			...
			args = frappe._dict(
				{
					...
					"attendance_end_date":self.attendance_end_date,
...
def get_filter_condition(filters):
	cond = ""
	for f in ["company", "branch", "parent_department", "department", "designation"]:
		if filters.get(f):
...
def get_emp_list(sal_struct, cond, end_date, payroll_payable_account):
	return frappe.db.sql(
		"""
			select
				distinct t1.name as employee, t1.employee_name, t1.parent_department, t1.department, t1.designation
			from
