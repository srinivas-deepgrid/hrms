...
frappe.ui.form.on("Salary Slip", {
	...
	attendance_start_date: function(frm) {
		if (frm.doc.attendance_start_date) {
			frm.trigger("set_attendance_end_date");
		}
	},

	attendance_end_date: function(frm) {
		if (frm.doc.employee && frm.doc.start_date && frm.doc.end_date && frm.doc.attendance_start_date && frm.doc.attendance_end_date) {
			return frappe.call({
	...
	set_attendance_end_date: function(frm) {
		frappe.call({
			method: 'hrms.payroll.doctype.payroll_entry.payroll_entry.get_end_date',
			args: {
				frequency: frm.doc.payroll_frequency,
				start_date: frm.doc.attendance_start_date
			},
			callback: function (r) {
				if (r.message) {
					frm.set_value('attendance_end_date', r.message.end_date);
				}
			}
		});
	},
	...
	payroll_frequency: function(frm) {
		...
		frm.set_value('attendance_end_date', '');
	},
